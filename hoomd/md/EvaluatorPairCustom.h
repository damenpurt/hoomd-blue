...
#ifndef __PAIR_EVALUATOR_CUSTOM_H__
#define __PAIR_EVALUATOR_CUSTOM_H__
...
/*! \file EvaluatorPairCustom.h
    \brief Defines the pair evaluator class for Custom potentials
...
class EvaluatorPairCustom
    ...
    public:
        ...
        */
        DEVICE EvaluatorPairCustom(Scalar _rsq, Scalar _rcutsq, const param_type& _params)
            : rsq(_rsq), rcutsq(_rcutsq), cu1(_params.x), cu2(_params.y)
            {
            }
        ...
        */
        DEVICE bool evalForceAndEnergy(Scalar& force_divr, Scalar& pair_eng, bool energy_shift)
            ...
            if (rsq < rcutsq && cu1 != 0)
                ...
                force_divr= r2inv * r6inv * (Scalar(12.0)*cu1*r6inv - Scalar(6.0)*cu2);

                pair_eng = r6inv * (cu1*r6inv - cu2);

                if (energy_shift)
                    {
                    Scalar rcut2inv = Scalar(1.0)/rcutsq;
                    Scalar rcut6inv = rcut2inv * rcut2inv * rcut2inv;
                    pair_eng -= rcut6inv * (cu1*rcut6inv - cu2);
                    }
                return true;
                }
            else
    ...
    protected:
        ...
        Scalar cu1;     //!< lj1 parameter extracted from the params passed to the constructor
        Scalar cu2;     //!< lj2 parameter extracted from the params passed to the constructor
    };


#endif // __PAIR_EVALUATOR_CUSTOM_H__
